...
- 下拉菜单、提示卡、确认框和移动端底部 Sheet 都应复用同一个应用级浮层机制；由它统一处理层级、焦点回收、`Escape`、点击外部关闭和滚动锁定。页面不能各自复制一套关闭逻辑。
- 触控断点下，把依赖 hover 的浮层改为显式可点击入口；内容较长或包含多个操作时使用有可访问名称的 Sheet，而不是挤在窄 Popover 里。
- 浮层内的业务提交、撤销和错误状态仍属于领域组件；共享浮层只负责显示生命周期和键盘可达性。这样新增页面不会把焦点管理和业务状态耦合在一起。
- 每个代表性旅程至少在桌面、窄屏触控和键盘操作下各走一遍。布局截图或一次静态构建不足以证明浮层可用。
